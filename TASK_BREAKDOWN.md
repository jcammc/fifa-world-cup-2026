...
Dependency: T-025.
Status: PARTIAL (Sprint 3) — flag, name, group, ranking, manager rendered. Average age, squad value, WCs won not yet populated from data.
...
Dependency: T-027.
Status: COMPLETE (Sprint 3) — sorts by caps desc when no consensus score (approved amendment).
...
Status: COMPLETE (Sprint 3) — implemented in js/charts.js with 5-axis SVG, viewBox 280×280, rings at 25/50/75/100%.
...
Status: COMPLETE (Sprint 3) — position groups used as layout sections (Goalkeepers/Defenders/Midfielders/Forwards), each with its own squad-grid.
...
- Mobile touch scroll
Dependency: T-034.
Status: COMPLETE (Sprint 3) — position-group model implemented (see blueprint §11). Observer root is #app-content scroll container, not viewport.
...
Status: PARTIAL (Sprint 3) — photo, name, position badge, club, age, caps, goals, bio implemented. Ranking breakdown and similar players deferred to later sprint.
