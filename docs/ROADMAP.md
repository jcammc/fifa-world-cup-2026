...
**Category:** Genuinely new feature (data sourcing + build) · **Status:** Infrastructure COMPLETE (2026-07-06/07). Pipeline, schema, generation script, all three UI surfaces, validation, and tests are built and verified. Of the 4 manual components, **Awards is now fully researched (254/286 players, all 11 teams — 2026-07-07)**; Transfermarkt and EA remain untouched, gated on the user supplying raw values (agent-side fetching confirmed blocked for both — see "Acquisition strategy revision" below). Every in-scope player is still `provisional` until all 4 components land.
