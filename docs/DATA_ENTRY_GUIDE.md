...
## SECTION 20 — RANKING SYSTEM: MANUAL RAW-VALUE ENTRY WORKFLOW (Sprint 39)

`data/rankings.json` holds a consensus score per player, built from 4 weighted components (Transfermarkt value, EA rating, Awards, Media pageviews) plus an automated Form score. Full design in `docs/plans/2026-07-06-ranking-system-design.md`; current completeness in `docs/ROADMAP.md`'s Sprint 39 retrospective.

**Never hand-edit `data/rankings.json` directly.** Every manually-sourced value goes in through `scripts/import-ranking-raw.mjs` (`npm run import-ranking-raw`), which bulk-imports one team's worth of values at a time from a pasted CSV (no header row) via stdin, then `npm run generate-rankings` recomputes the derived 0–100 scores fresh from the raw values every time it runs.

### Why manual at all — what's automated vs. not

Media (`mediaPageviews`) and part of Awards (`worldCupWinner`, `ballonDorTier` winner-tier, `uefaPoty`, `wcGoldenBall`) are fully automated by `scripts/gather-rankings-signals.mjs` — no manual entry needed for those. Transfermarkt and EA are **fully manual**: agent-side fetching was tested directly against both live sites and confirmed blocked (Transfermarkt 403s any automated request; EA's site only renders ~15-20 global "featured" players in static content, and both scriptable mirrors return 403 too). The rest of Awards (individual-award tiers beyond a plain win, Champions League win counts, domestic title counts) is manual because it lives in free Wikipedia prose, not a structured field — reliably parsing free text into a specific tier isn't achievable without risking a silent misclassification.
...
- `ballonDorTier`: blank | `winner` | `top3` | `top10` — only set on **explicit placement evidence** in the honours text (e.g. an explicit "3rd place: 2022" citation). Never inferred from a sub-award — the Kopa Trophy (U21) and Gerd Müller Trophy (top scorer) don't imply a specific overall Ballon d'Or placement.
- `fifaBestPlayer`/`uefaPoty`/`totyEaFc`: blank | `true` | `false`.
- `clWins`/`domesticTitles`: blank | integer.
- `sourceUrl` (8th column, Awards only, optional): overrides the shared `--source` flag for that one row. Transfermarkt/EA share one accurate source per team (one squad page shows all ~26 players), but Awards research is per-player — each player has their own Wikipedia article.
- A **blank cell** means "not researched" and is left untouched — never coerced to `0`/`false`. For Awards, a player's row appearing in the CSV at all marks `awardsRaw` as researched (a real object) even when every column is blank, so "checked, genuinely has none of these" correctly resolves to a computed score of `0`, not stuck on `null` forever.
- Never overwrites a raw field that's already non-null, unless `--force` is passed (a deliberate correction).

### Conventions for tabulating Awards data, established across 11 teams' worth of research

- **`clWins`** = **UEFA Champions League wins only.** Excludes continental-but-non-UEFA equivalents a player might have (CAF Champions League, Copa Libertadores, AFC Champions League) even though they're phrased identically ("Champions League") in the source text.
- **`domesticTitles`** = **top-flight LEAGUE championship wins only.** Excludes all cup competitions (Copa del Rey, FA Cup, DFB-Pokal, etc.) and supercups (Supercopa, Trophée des Champions, etc.); excludes second/third-tier divisions (English EFL Championship, Norwegian 1. divisjon, Belgian Challenger Pro League, Ligue 2, Serie B, Segunda División); excludes MLS Cup and the Supporters' Shield deliberately (structural playoff-format ambiguity, not a simple points-table league title); excludes sub-national competitions that read like a "league" but aren't the actual national top flight (Brazil's Campeonato Carioca/Paulista are state championships — the national league is the Campeonato Brasileiro Série A).
- **`totyEaFc`** = `true` only for a genuine FIFPro World11 / "FIFA Men's World 11" selection — excludes league-specific "Team of the Season", and excludes independent media picks (France Football's World XI, ESM Team of the Year) that use similar naming but aren't the real basis for EA's in-game TOTY.
- **`fifaBestPlayer`** = `true` only for an actual "The Best FIFA Men's Player" / historical "FIFA World Player of the Year" win — explicitly distinct from the separate "Best FIFA Men's Goalkeeper" award.
- Youth/U-level competitions and Olympics are excluded from every field.
- A player with no detected Honours section is **omitted from the CSV entirely**, never defaulted to `false`/`0` — flag them by name in the commit message for individual follow-up rather than guessing.

### Research workflow, one team at a time

1. `node scripts/dump-player-honours.mjs --team <team> > /tmp/<team>-honours.txt` — pulls each player's raw Wikipedia Honours-section text (both "Honours" and the American-English "Honors" spelling are matched). This is a research aid only — it never parses or guesses a value, it just gets the raw text in front of a human faster than fetching each article by hand.
2. Read the dump and hand-tabulate each player's row per the conventions above.
3. `node scripts/import-ranking-raw.mjs --field awards --team <team> --source "manual research (Wikipedia)" < values.csv`
4. `npm run generate-rankings` to recompute derived scores, `npm run validate` and `npm test` to confirm a clean state, then commit.

**A resolved player with an unexpectedly empty or implausible-looking dump is worth a second look, not an automatic omission** — Sprint 39 found and fixed two real bugs this way: Wikipedia's American-English "Honors" spelling (a 100% miss rate for an entire squad before the fix) and `resolveArticleTitle()` accepting a disambiguation page as a real match when a player's plain name collides with someone more famous (confirmed cases: Colombia's Luis Suárez direct-matching the Uruguayan Luis Suárez; Spain's Rodri and Gavi matching list pages instead of their own articles). If a well-known player comes back with no Honours section at all, or a suspiciously thin one, spot-check by resolving their Wikipedia title manually before concluding the data genuinely doesn't exist.
