...
| `data.js` | Complete | DataManager singleton, #cache Map, all loaders including loadSearchIndex() |
...
| `modules/search-overlay.js` | **Complete** | Ctrl+K or nav button trigger, live filtering, diacritic normalisation, team/player results, player deep-link nav |
...
### Sprint 13 — recentForm population
Populate `recentForm: ["W","D","W","W","L"]` for all 48 countries in countries.json (last 5 international results before the tournament). Source from Wikipedia nation team pages. Only use verified match data — never from memory. At least 2-3 WC group stage results per team now exist as data points.

### Sprint 14 — Player photos / Club badges
`gather-photos.js` stub is ready. Need a sourcing strategy (Wikipedia image API or wikidata). The `profile-panel.css` has fallback avatar/badge CSS already.
