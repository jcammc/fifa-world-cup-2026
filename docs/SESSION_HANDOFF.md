...
| `gather-match-events.mjs` | **New (Sprint 29)** | Wikipedia scraper for group + knockout match events (goals, MOTM, formations). Fetches `{{#invoke:football box\|main}}` templates from Wikipedia group pages and round pages via `action=parse&prop=wikitext`. Idempotent — skips fixtures already in `match-events.json`. **429 retry:** `fetchWikitext()` retries up to 3× with 5/10/15s backoff. **FIFA code map:** 48-team `FIFA_TO_ID` mapping (extended with CZE, CUW, JOR). **Per-box result categories:** `populated`, `skipped`, `rate_limited`, `page_missing`, `fetch_error`, `incomplete_template`, `no_fixture_match`, `unknown_fifa_code`. Grouped summary table printed after run. **Known gaps:** Group K (6 matches) consistently rate-limited; Group D rounds 2-3 (4 matches) hit Wikipedia LST (section transclusion) — boxes contain only date param. Run: `npm run gather-match-events`. Idempotent: re-run to pick up newly available data as tournament progresses. |

**npm scripts** (`package.json`):
```
npm run sync-data            → node scripts/sync-data.mjs           (pull live scores/standings from API)
npm run gather-match-events  → node scripts/gather-match-events.mjs  (Sprint 29 — scrape Wikipedia match events)
...
| `match-events.json` | **New (Sprint 29)** — object-keyed by fixture ID. 78/104 entries populated. Gaps: Group K (6 matches, rate-limited), Group D R2-R3 (4 matches, Wikipedia LST issue), R16/QF/SF/Final (not yet played). Schema: `{ version, lastUpdated, data: { [fixtureId]: { events[], motm, homeFormation, awayFormation, homeStarting[], awayStarting[] } } }`. Loaded via `DataManager.loadMatchEvents()` → `#loadRaw()`. Run `npm run gather-match-events` to populate more entries as tournament progresses. |
...
```json
{
  ...
  "data": {
    "i-r1-fra-sen": {
      "events": [
        { "type": "goal", "minute": "14",   "teamId": "france",  "scorer": "Mbappé", "assistBy": null },
        { "type": "goal", "minute": "45+2", "teamId": "senegal", "scorer": "Diatta",  "assistBy": null }
      ],
      "motm": "Kylian Mbappé",
      "homeFormation": "4-3-3",
...
**Event types:** `goal` | `substitution` | `yellow_card` | `red_card` (only `goal` is currently populated from Wikipedia; others reserved for future use).
...
- `match-events.json` → `DataManager.loadMatchEvents()` — `{ version, lastUpdated, data: { [fixtureId]: {...} } }` **(Sprint 29)**
