...
- `docs/LIVE_DATA_PLAN.md` — live data pipeline implementation — football-data.org + Netlify Blob Store + Scheduled Functions. **Implemented Sprint 25.** See §11 of that doc ("Architecture Redesign — Cache-Aside Pipeline") for deviations from the original plan.
...
| Sprint 32 | Data gap closed: Group K match events re-gathered — all 6 fixtures now fully populated (events + MOTM). **P1+P5 (Match Centre tab strip + manager links):** sticky scroll-to-anchor tab strip (`#mc-tab-strip`) driven by IntersectionObserver — FT matches get Match/Context/Teams tabs, upcoming get Preview/Lineups/Teams; manager names in Match Centre now link to `#manager/{countryId}`. **P2 (H2H schema split):** `headToHead` field split into `matchStory` (FT — past-tense narrative), `headToHead` (upcoming — historical framing), and new `headToHeadStats` (2-col label/value stat grid via `#buildH2HStatsGrid`); prose now collapses into a `<details>` element; `gather-head-to-head.mjs` writes `matchStory` for FT fixtures and migrates existing entries on re-run. **P3 (SVG lineup graphic):** `Charts.renderLineup(container, formation, players)` — portrait-pitch SVG with player circles (shirt + surname), GK at bottom; replaces the old text-column lineup in `#buildPreviousLineupSection` (text column kept as `aria-hidden` fallback). **P4+P6 (manager honours + profile page + player descriptions):** all 92 honour entries in `managers.json` gained a `club` field (up from 54); missing major honours filled for several managers (Ancelotti, Deschamps, Koeman, Solbakken, etc.); new dedicated **Manager Profile page** (`js/modules/manager-page.js`, route `#manager/{countryId}`) — photo, bio, WC record, honours with club, career accordions, Wikipedia link; `gather-guardian-bios.mjs` now writes to a new `player.description` field (short Guardian-sourced blurb) instead of overloading `bio`; `description` is preferred over `bio` in hero cards, profile panel (full bio still available in a `<details>`), and captain cards. | **COMPLETE** |
...
| `router.js` | Complete | Hash routing, all current routes wired. **Sprint 26:** `match/` route added (before player deep-link loop). **Sprint 27:** `best-thirds` route added. **Sprint 32:** `manager/{countryId}` route added (ManagerPage module). Nav active state now treats `match/*`, `best-thirds`, and `manager/*` as Tournament Centre / team-page context (highlights `#tournament` or the relevant team nav link). |
...
| `charts.js` | Complete | `Charts.renderRadar(container, data)` — 5-axis SVG spider chart. `data`: `{ attack, midfield, defence, goalkeeping, depth }` all 0–100. **Sprint 32:** `Charts.renderLineup(container, formation, players)` — pure SVG portrait pitch with player circles (shirt number + surname), pitch markings, GK positioned at bottom. Used by Match Centre's previous-lineup section. |
...
- `nav.css` — top nav bar
- `team-page.css` — TeamPage tabs, squad layout, manager accordion styles (`.tp-manager__tenure`, `.tp-manager__details`, `.tp-manager__toggle`, `.tp-manager__expanded`, `.tp-mgr-career`, `.tp-mgr-honours`, `.tp-mgr-honour__role--manager/--player`). **Sprint 32:** `.tp-hero-card__desc` (hero card description text), `.tp-mgr-honour__club` (club name in honour chip).
- `manager-page.css` — **New (Sprint 32).** Dedicated stylesheet for the Manager Profile page (`#manager/{id}`): header (photo/initials, name, meta), bio section, WC record, honours list, career accordion, player-clubs list, Wikipedia link.
...
- `match-centre.css` — **New (Sprint 26), V2 (Sprint 27), V3 (Sprint 29), V4 (Sprint 31), V5 (Sprint 32).** `mc-` namespace. Base: page wrapper, back link, header, teams/flags, score/time, meta. Sprint 27 V2: `.mc-section` (border-top sections), form strips (`.mc-form__dot--w/d/l`), stake cards (`.mc-stake__card`, `.mc-stake__pos`, `.mc-stake__scenarios`, `.mc-stake__scenario--win/draw/loss`, `.mc-stake__chip--qualified/eliminated/leading/contention/danger`), radars (`.mc-radars` 2-col, stacks 1-col ≤480px), managers, captains. `.mc-standings` has `padding-bottom` (moved to mid-page in V2 reorder). Sprint 29 V3: `.mc-goals`, `.mc-goal-row` (2-col grid home/away), `.mc-goal-row__home/away`, `.mc-goal__assist`, `.mc-goals__none`, `.mc-motm`. Sprint 31 V4: `.mc-event-row` (3-col grid `1fr 40px 1fr`), `.mc-event-row__home/min/away`, `.mc-event-row--goal/yellow-card/red-card/substitution`, `.mc-event__yc` (amber CSS square 10×13px), `.mc-event__rc` (red CSS square 10×13px), `.mc-event__sub-on/sub-off/name`, `.mc-hth` (left-bordered italic blockquote), `.mc-lineups` (2-col grid), `.mc-lineup__pos-group/pos-label/players`, `.mc-susp` (2-col grid), `.mc-susp__player/at-risk/suspended`, `.mc-captain__bio` (xs italic muted text). Sprint 32 V5: `.mc-tab-strip` (sticky below nav), `.mc-tab`, `.mc-tab--active`, `.mc-tab-group` (`scroll-margin-top` for anchor targets), `.mc-manager__name--link`, `.mc-h2h-stats` (2-col label/value grid), `.mc-hth-details` (collapsible `<details>`), `.mc-hth-prose`, `.mc-lineup-panel`, `.mc-lineup-canvas` (SVG pitch mount point).
- `theme.css` — **Sprint 32 addition:** `--color-lineup-pitch` and `--color-lineup-pitch-bg` custom properties for both light/dark themes (used by `Charts.renderLineup()`).
...
| `generate-player-bios.js` | **Deleted (Sprint 40)** | Superseded by `gather-guardian-bios.mjs`. |
| `generate-rankings.js` | **Complete (Sprint 39)** | Orchestrator for the ranking system — seeds stub entries for `data/ranking-scope.json`'s 11 teams, computes Form from `match-events.json`, derives all 4 manual components (`transfermarkt`/`ea`/`awards`/`media`) from their raw fields via `scripts/lib/ranking-formula.mjs`, computes renormalized `consensus`. Run: `npm run generate-rankings`. |
| `update-standings.js` | **Deleted (Sprint 40)** | Superseded by `sync-data.mjs`. |
| `dump-player-honours.mjs` | **New (Sprint 39)** | Research aid for manual Awards data entry — pulls each in-scope player's raw Wikipedia Honours-section text for reading/transcription. Not an extraction script; never parses or guesses a value. Run: `npm run dump-player-honours -- --team <team>`. |
| `import-ranking-raw.mjs` | **New (Sprint 39)** | Bulk-imports one team's researched raw ranking values (`--field transfermarkt\|ea\|awards`) from a pasted CSV via stdin into `data/rankings.json`, instead of hand-editing JSON. Conservative by default — never overwrites a non-null raw field without `--force`. See `docs/DATA_ENTRY_GUIDE.md` for the CSV format and worked conventions. |
| `gather-rankings-signals.mjs` | **New (Sprint 39)** | Automates the ranking signals that are genuinely fetchable: Wikimedia Pageviews for Media, World Cup winner / Ballon d'Or (winner-tier) / UEFA POTY / World Cup Golden Ball detection for Awards via Wikipedia infobox + Wikidata `P166` claims. Run: `npm run gather-rankings-signals`. |
...
| `gather-guardian-bios.mjs` | **New (Sprint 31), updated (Sprint 32)** | Attempts to fetch The Guardian's WC 2026 player guide JSON for bio data. CDN and page fetches are all blocked by their anti-bot CDN — **automated fetch always fails**. **Manual fallback (required):** (1) open the Guardian URL in a browser; (2) open DevTools console; (3) run `copy(JSON.stringify(window.__NEXT_DATA__))`; (4) paste into a new file `data/guardian-raw.json`; (5) run `npm run gather-guardian-bios` — script reads the local file, fuzzy-matches player names, writes text to the **`description`** field (Sprint 32 — previously wrote to `bio`; `bio` is left for longer-form text from other sources) in each player file. Run: `npm run gather-guardian-bios`. |
...
npm run pre-deploy           → validate && gather-guardian-bios && build-search-index (Sprint 40 — generate-bios/generate-rankings removed, since generate-rankings.js isn't a re-run-on-every-deploy step)
```
...
| `managers.json` | **New (Sprint 22), enriched (Sprint 32)** — 48 entries, object-keyed by countryId. Each: `career[]` (managerial appointments, chronological, `{ role, club, years }`), `playerClubs[]` (max 3 notable clubs as player, empty array if never played professionally), `honours[]` (major titles only, `{ title, year, role: "Manager"\|"Player", club }` — **`club` field added Sprint 32**, now present on all 92 honour entries, up from 54; several managers' missing major honours were also filled in). Lazy-loaded per team via `DataManager.loadManager(countryId)`. Also backs the new **Manager Profile page** (`#manager/{countryId}`, Sprint 32). |
...
| `match-events.json` | **New (Sprint 29), enhanced (Sprint 31)** — object-keyed by fixture ID. Group K gap (6 matches, previously rate-limited) **closed** — all 6 now fully populated with events + MOTM. Group D R2-R3 (4 matches): Wikipedia LST issue, still open. R16/QF/SF/Final: populate as matches are played. Schema: `{ version, lastUpdated, data: { [fixtureId]: { events[], motm, homeFormation, awayFormation, homeStarting[], awayStarting[], homeSubs[], awaySubs[] } } }`. All four event types now parsed from Wikipedia lineup tables. Loaded via `DataManager.loadMatchEvents()` → `#loadRaw()`. |
| `match-previews.json` | **New (Sprint 31), schema split (Sprint 32)** — object-keyed by fixture ID. 88 entries (all 48 group fixtures + all 32 R32 matches). R16/QF/SF/Final not yet on Wikipedia. Schema: `{ version, lastUpdated, data: { [fixtureId]: { matchStory?: string, headToHead?: string, headToHeadStats?: object, source: "Wikipedia" } } }`. **Sprint 32:** `headToHead` field split — FT fixtures get `matchStory` (past-tense narrative, shown under "Match Story"), upcoming fixtures keep `headToHead` (historical framing, shown under "World Cup History"); new `headToHeadStats` object renders as a 2-col label/value grid. `gather-head-to-head.mjs` migrates existing entries to `matchStory` on first re-run for FT matches. Loaded via `DataManager.loadMatchPreviews()` → `#loadRaw()`. Run `npm run gather-head-to-head` to refresh or pick up new round pages. |
...
| `rankings.json` | **Sprint 39 — infrastructure complete, Awards research complete (confirmed on two independent audit passes).** 286 players across 11 in-scope teams (`data/ranking-scope.json`). `mediaPageviews` populated for 285/286 (auto-gathered). `awardsRaw` populated for 260/286 (manually researched via Wikipedia, per-player `rawProvenance`) — 26 confirmed genuine gaps (no Honours section on the player's own article, re-verified after the resolver fixes). `transfermarktValueEUR`/`eaRatingRaw` still null for all 286 — gated on the user supplying raw values per team, agent-side fetch confirmed blocked for both. Every entry still `provisional: true` until all 4 components land. |
...
**Optional sparse field, not yet populated in any squad** (added Sprint 32, awaiting a `gather-guardian-bios` run with a manually-extracted `guardian-raw.json`):
```json
"description": "Short Guardian-sourced blurb"
```
`description` is preferred over `bio` in hero cards, profile panel, and Match Centre captain cards once populated; `bio` (longer form, from other sources) remains available in a `<details>` element as a fallback/expansion.
...
#manager/{countryId}  ManagerPage        { countryId: string }   ← Sprint 32
...
**Current phase: Round of 16 complete, Quarter-finals underway (as of Sprint 34 Pass 2, 2026-07-06).** R32 (June 28 – July 4) and R16 matchdays 1–2 are fully resolved; QF has 2 of 4 pairings known (`qf-m1`: France v Morocco, `qf-m3`: Norway v England). Scores and team IDs update automatically via `live-data.mjs` in production; local/offline work uses `npm run sync-data` + manual `update-knockout.js` propagation (see below — the automatic team-matching in both pipelines has a known gap when two TBD slots share a kickoff date, see Known Issues).
...
- ~~`data/rankings.json` empty — Rankings component not implemented.~~ — **Implemented Sprint 39.** Pipeline, schema, all 3 UI surfaces are done. Manual raw-value coverage as of 2026-07-08: Transfermarkt 286/286 (complete, all 11 teams), Awards 260/286 (complete, second audit pass confirmed the remaining 26 are genuine gaps), Media 285/286, EA 0/286 (not started — next phase, same team-by-team CSV workflow). See Sprint 39 retrospective in `docs/ROADMAP.md`.
...
- ~~`scripts/update-standings.js` and `scripts/generate-player-bios.js`, `scripts/generate-rankings.js` are stubs.~~ — **Resolved.** `update-standings.js`/`generate-player-bios.js` deleted Sprint 40 (superseded by `sync-data.mjs`/`gather-guardian-bios.mjs`). `generate-rankings.js` fully implemented Sprint 39 — see the scripts table above.
...
- **Knockout bracket: round-level confirmation-tick gate, connector-line array-order mismatch, and duplicated sync/live-data propagation logic (found 2026-07-06, plan written, not yet fixed).** Three related architectural issues in the bracket: (1) the green "confirmed" tick on a team's slot is suppressed only when *every* match in its round is fully resolved, not per-match — so a single still-open sibling match keeps the tick showing on every other match in an otherwise-resolved round (live-confirmed moving from R16 to QF as data caught up during Sprint 34 Pass 2); (2) `#positionBracket()`'s SVG connector lines in `js/modules/knockout-bracket.js` are positioned by array order, not the actual propagation relationships in `scripts/update-knockout.js`'s `PROPAGATION` map — verified wrong for 7/8 R16 slots and 2/4 QF slots (team names in each card are still correct; only the connector lines misrepresent which match feeds which); (3) `scripts/sync-data.mjs` and `netlify/functions/live-data.mjs` independently reimplement the same fragile team-pair/date-fallback matching, which live-misfired twice in Sprint 34 Pass 2 (`r16-m7`/`r16-m8`, `qf-m3`/`qf-m4` — same kickoff date, ambiguous, required manual `update-knockout.js` propagation). Full investigation, root causes, and recommended fix (shared `js/bracket-topology.js` propagation graph, per-match tick rule, consolidated merge function) written up in `docs/ROADMAP.md` Sprint 42 — plan only, not yet implemented, awaiting greenlight.
