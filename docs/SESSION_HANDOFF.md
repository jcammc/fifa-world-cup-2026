...
| Sprint 20 — Audit | Photo Quality Audit: enumerated all 130 Pass-1 recoveries by filename. Found 22 confirmed false positives (20 from Pass 1, 2 from original pass): logos (KRC Genk, Uzbekistan league/cup, Jordan team ×2), non-persons (Qasem Soleimani, Iran protests 2026), wrong-player assignments (Erick Davis, Meschack Elia, Cristian Borja, Stiven Mendoza, Behruz Karimov, Al Mehdi Al Harrar ×2, Dostonbek Khamdamov, Michael Amir Murillo, Roderick Miller, Ahmed Al-Kaf), two duplicate-URL Panama pairs, Argentina GK's photo on Uruguay's Emiliano Martínez, and Wollacott's photo on Ghana Harrison. All 22 nulled out. Coverage adjusted: 974 → 952/1248 (76.3%, net +108 over original 844). `isSuspicious()` strengthened: now also blocks `logo`, `_crest`, `_badge`, `_emblem`, `_shield`, `_coat_of_arms`, `federation`, `association` tokens and `.svg.png` thumbnails. Post-batch duplicate-URL rejection added to `runRetryPass()`. | **COMPLETE** |
...
| `gather-photos.js` | **Complete** | Two modes: `RETRY_NULLS=false` (normal — exact-title lookup for undefined entries); `RETRY_NULLS=true` (Pass 1 — Search API retry for null entries). `isSuspicious()` rejects placeholders, logos, crests, SVG thumbnails (`.svg.png`), federation/association images. `runRetryPass()` has post-batch duplicate-URL rejection (if two players resolve to the same image URL, both are skipped). 45s cooldown + 2s per-request delay between phases to avoid Wikipedia rate limits. Run: `node scripts/gather-photos.js`. Set `RETRY_NULLS=false` for the next Pass 2 (Wikidata) or a new normal-mode run. |
...
| `player-photos.json` | **952/1,248 URLs confirmed correct (76.3%)**, 296 nulls. Covers all 1,248 players. History: original pass 844/1248 (67.6%) → Pass 1 (Search API retry) 974/1248 (78.0%) → Quality Audit removed 22 false positives → 952/1248 (76.3%). Schema: `data` is an Object keyed by player ID, value = URL string or `null`. `null` = confirmed no image (or failed quality check). `undefined` = not yet queried (no entries of this type remain). Pass 2 (Wikidata P18 image fallback) is ready to run — `isSuspicious()` now blocks logos/SVGs/crests, and post-batch duplicate-URL rejection is in place. |
