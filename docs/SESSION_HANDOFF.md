...
| `rankings.json` | **Sprint 39 — infrastructure complete, Awards research complete.** 286 players across 11 in-scope teams (`data/ranking-scope.json`). `mediaPageviews` populated for 285/286 (auto-gathered). `awardsRaw` populated for 254/286 (manually researched via Wikipedia, per-player `rawProvenance`) — 32 confirmed genuine gaps (no Honours section on the player's own article). `transfermarktValueEUR`/`eaRatingRaw` still null for all 286 — gated on the user supplying raw values per team, agent-side fetch confirmed blocked for both. Every entry still `provisional: true` until all 4 components land. |
...
- ~~`data/rankings.json` empty — Rankings component not implemented.~~ — **Implemented Sprint 39.** Pipeline, schema, all 3 UI surfaces, and Awards manual research (254/286 players, all 11 in-scope teams) are done. Transfermarkt/EA raw values still pending — see Sprint 39 retrospective in `docs/ROADMAP.md`.
